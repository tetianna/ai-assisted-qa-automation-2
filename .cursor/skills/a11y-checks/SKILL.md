---
name: a11y-checks
description: Adds @axe-core/playwright accessibility scans and keyboard coverage when generating or reviewing Playwright tests for new pages or components. Apply whenever creating, extending, or reviewing UI tests — even if the user does not mention accessibility, a11y, or axe.
---

# Accessibility Checks

Every Playwright test for a new page or component must include **axe-core coverage plus a keyboard path axe cannot cover**. This does not require the user to ask. Prefer dedicated specs under `tests/<feature>.a11y.spec.ts` (see programs page below).
...
- Assert with `await expect(results.violations).toEqual([])` — never bare `assert` or manual length checks.
- Optionally use `formatViolations` / `recordViolations` from `tests/helpers/a11y.ts` for clearer failure output.
- Scope with `.include()` / `.exclude()` only if a third-party widget is noisy, and **comment why**. Prefer full-page scans for page-level coverage; for a modal/component under test, `.include()` from a role-based POM helper (e.g. `NewProgramModal.axeIncludeSelector()`) is fine.

**Auth:** use `fixtures/cleanup.fixture` for authenticated routes (`storageState`). Use `@playwright/test` only for unauthenticated pages (e.g. login).
...
If the scan finds real violations: report them and stop. **Never use `.disableRules()` to go green.**

When axe finds real issues:
...
- Keep axe scans and keyboard asserts in **specs**, not POMs. POMs may expose `axeIncludeSelector()` helpers only.
