...
async function gotoTodoApp(page: Page) {
...
function todoItem(page: Page, label: string, index = 0): Locator {
  return todoItems(page)
    .filter({ has: page.locator('label', { hasText: label }) })
    .nth(index);
}

async function toggleTodo(page: Page, label: string, index = 0) {
  await todoItem(page, label, index).locator('.toggle').click();
}

async function deleteTodo(page: Page, label: string, index = 0) {
  const item = todoItem(page, label, index);
  await item.hover();
  await item.locator('.destroy').click();
}

function buildLongTodoText(length = 500): string {
...
test.describe('TodoMVC — Positive flows', () => {
  ...
  test('TC-002: multiple todos are retained in creation order', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    ...
    await expect(todoItems(page).locator('.toggle')).toHaveCount(3);
  ...
  test('TC-003: todo is marked completed when the user toggles its checkbox', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    ...
    await expect(item.locator('label')).toHaveText('Buy groceries');
  });

  test('TC-004: completed todo returns to active state when toggled again', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    await toggleTodo(page, 'Buy groceries');
    await toggleTodo(page, 'Buy groceries');
  ...
  test('TC-005: todo is removed from the list when the user deletes it', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    await addTodo(page, 'Walk the dog');
  ...
  test('TC-006: user can add, complete, and delete the same todo in one session', async ({
    page,
  }) => {
    ...
    await expect(todoItem(page, 'Schedule dentist appointment')).toHaveClass(/completed/);
    await expect(todoCount(page)).toHaveText('0 items left');
    ...
    await expect(todoCount(page)).toBeHidden();
...
test.describe('TodoMVC — Negative flows', () => {
  ...
  test('TC-007: empty submission does not create a todo', async ({ page }) => {
  ...
  test('TC-009: deleting one item does not remove sibling todos', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    await addTodo(page, 'Walk the dog');
    await addTodo(page, 'Pay electricity bill');
    ...
    await expect(todoItem(page, 'Pay electricity bill')).toBeVisible();
    await expect(todoItem(page, 'Buy groceries')).not.toHaveClass(/completed/);
    await expect(todoItem(page, 'Pay electricity bill')).not.toHaveClass(/completed/);
    await expect(todoCount(page)).toHaveText('2 items left');
  });

  test('TC-010: completing a todo does not delete it from the list', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    await toggleTodo(page, 'Buy groceries');
    ...
    await expect(item).toHaveClass(/completed/);
    await item.hover();
    await expect(item.locator('.destroy')).toBeVisible();
  });

  test('TC-011: Enter pressed outside the new-todo field does not add todos', async ({ page }) => {
...
test.describe('TodoMVC — Edge cases', () => {
  ...
  test('TC-012: todo text containing special characters is stored and displayed correctly', async ({
    page,
  }) => {
    ...
    await expect(item.locator('label')).toHaveText(text);
  ...
  test('TC-013: duplicate todo titles are allowed as separate list entries', async ({ page }) => {
    await addTodo(page, 'Buy groceries');
    await addTodo(page, 'Buy groceries');
    ...
    await expect(todoItem(page, 'Buy groceries', 0)).toHaveClass(/completed/);
    await expect(todoItem(page, 'Buy groceries', 1)).not.toHaveClass(/completed/);
    await expect(todoCount(page)).toHaveText('1 item left');
  ...
  test('TC-014: very long todo text is accepted and supports complete and delete', async ({
    page,
  }) => {
    ...
    await expect(item).toBeVisible();
    await expect(item.locator('label')).toHaveText(longText);
  ...
  test('TC-015: single-character todo is valid', async ({ page }) => {
    await addTodo(page, 'A');

    const item = todoItem(page, 'A');
    await expect(item.locator('label')).toHaveText('A');
    await expect(todoCount(page)).toHaveText('1 item left');
  ...
  test('TC-016: unicode and emoji characters display correctly in todo labels', async ({
    page,
  }) => {
  ...
  test('TC-017: leading and trailing whitespace is trimmed on create', async ({ page }) => {
    await addTodo(page, '  Trim me  ');

    await expect(todoItems(page)).toHaveCount(1);
    await expect(todoItem(page, 'Trim me')).toBeVisible();
    await expect(todoItem(page, 'Trim me').locator('label')).toHaveText('Trim me');
  ...
  test('TC-018: HTML-like input is shown as plain text, not executed', async ({ page }) => {
    ...
    await expect(todoItem(page, "<script>alert('xss')</script>")).toBeVisible();
    await expect(todoItem(page, '<b>Bold todo</b>')).toBeVisible();
    await expect(page.locator('.todo-list b')).toHaveCount(0);
  });

  test('TC-019: rapid sequential adds create distinct items without loss', async ({ page }) => {
    const tasks = ['Task 1', 'Task 2', 'Task 3', 'Task 4', 'Task 5'];
    for (const task of tasks) {
      await addTodo(page, task);
    }

    await expect(todoItems(page)).toHaveText(tasks);
    await expect(todoCount(page)).toHaveText('5 items left');
  });

  test('TC-020: delete last remaining todo returns app to initial empty state', async ({
    page,
  }) => {
    await addTodo(page, 'Buy groceries');
    await deleteTodo(page, 'Buy groceries');
