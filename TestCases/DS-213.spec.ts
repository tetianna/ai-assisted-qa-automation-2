...
import { hasNonAdminCredentials, uniqueName } from '../tests/helpers/didaxis';
...
test.describe('DS-213: Add user in Settings', () => {
  ...
  test.describe('Happy paths', () => {
    test('TC-001 — Admin navigates to Settings and opens add-user form', { tag: '@sanity' }, async ({ page }) => {
      const settingsPage = new SettingsPage(page);
    ...
    test('TC-002 — Admin successfully adds a new user with required fields', { tag: '@smoke' }, async ({
      page,
      trackUser,
    }) => {
      const settingsPage = new SettingsPage(page);
    ...
    test('TC-003 — Newly added user appears without manual page refresh', { tag: '@sanity' }, async ({
      page,
      trackUser,
    }) => {
      const settingsPage = new SettingsPage(page);
  ...
  test.describe('Negative', () => {
    test('TC-004 — Add user form rejects submission with empty email', { tag: '@regression' }, async ({ page }) => {
      const settingsPage = new SettingsPage(page);
      const modal = settingsPage.addUserModal;
    ...
    test('TC-005 — Add user form rejects duplicate email', { tag: '@regression' }, async ({ page, trackUser }) => {
      const settingsPage = new SettingsPage(page);
      const modal = settingsPage.addUserModal;
    ...
    test('TC-006 — Non-admin cannot access add-user functionality', { tag: '@regression' }, async ({ page }) => {
      test.skip(
  ...
  test.describe('Edge cases', () => {
    test('TC-007 — Add user with special characters in display name', { tag: '@regression' }, async ({
      page,
      trackUser,
    }) => {
      const settingsPage = new SettingsPage(page);
    ...
    test('TC-008 — Add user with maximum-length email', { tag: '@regression' }, async ({ page, trackUser }) => {
      const settingsPage = new SettingsPage(page);
    ...
    test('TC-009 — Cancel add-user form discards input', { tag: '@sanity' }, async ({ page }) => {
      const settingsPage = new SettingsPage(page);
      const modal = settingsPage.addUserModal;
