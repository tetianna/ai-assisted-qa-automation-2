...
Every sub-task **summary** must start with `Taya`, then the agent name:

```
Taya [Composer] <clear, specific defect description>
```

Example: `Taya [Composer] Program list shows stale data after editing program name`
...
- Sub-task **summary** must begin with `Taya` (e.g. `Taya [Composer] …`)
...
1. Read tool schemas before calling (`GetMcpTools`, then `CallMcpTool`).
2. In this repository, use `cloudId = "https://legionqaschool.atlassian.net"` — do **not** call `getAccessibleAtlassianResources`. Jira project key is **DS**.
3. Search duplicates with `searchJiraIssuesUsingJql` (`maxResults: 10`).
4. Create the sub-task with `createJiraIssue`:
   - `cloudId`: `"https://legionqaschool.atlassian.net"`
   - `projectKey`: `DS`
   - `issueTypeName`: `Sub-task`
   - `parent`: parent story key (e.g. `DS-2`)
   - `summary`: `Taya [Composer] <defect description>`
...
|---|---|
| Title | `summary` (must start with `Taya [Composer]`) |
